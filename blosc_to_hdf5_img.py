...
# 每次采集数据时要检查数据，检查shape,检查打印的数据具体数值是否连续，检查图像是否正常显示等。
...
def convert_dataset(blosc_dir, hdf5_path):
    ...
    with h5py.File(hdf5_path, "w") as h5file:
        for idx, fname in enumerate(blosc_files, start=0):
            ...
            print(f"[INFO] Writing {demo_name} <- {fname} (frames: {len(obs)})")
...
if __name__ == "__main__":
    ...
    BLOSC_DIR = r"init_rectangle_blue_weita"
    HDF5_FILE = r"init_rectangle_blue_weita/blue_weita_RGB.hdf5"
